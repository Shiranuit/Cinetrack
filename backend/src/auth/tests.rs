...
fn token_roundtrips_the_user_id() {
    ...
    let tok = token::issue(secret, uid, "sess-1", 900).unwrap();
    assert_eq!(token::verify(secret, &tok).unwrap(), (uid, "sess-1".to_string()));
}

#[test]
fn token_rejects_a_wrong_secret() {
    let tok = token::issue("secret-a", uuid::Uuid::now_v7(), "s", 900).unwrap();
    assert!(matches!(token::verify("secret-b", &tok), Err(AppError::Unauthorized(_))));
}

#[test]
fn token_rejects_tampered_and_malformed_tokens() {
    let tok = token::issue("secret", uuid::Uuid::now_v7(), "s", 900).unwrap();
