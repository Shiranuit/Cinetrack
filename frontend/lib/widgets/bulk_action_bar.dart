...
class BulkActionBar extends StatefulWidget {
  const BulkActionBar({super.key, required this.controller, this.onChanged, this.inLibrary = false});
...
class _BulkActionBarState extends State<BulkActionBar> {
  ...
  Future<int> _run(bool Function(SelItem) applies, Future<void> Function(ApiClient, SelItem) op) async {
    ...
    for (var i = 0; i < items.length; i += chunk) {
      ...
      final results = await Future.wait(batch.map((it) async {
        try {
          await op(api, it);
          return true;
        } catch (_) {
  ...
  Future<void> _act(bool Function(SelItem) applies, Future<void> Function(ApiClient, SelItem) op) async {
    if (_busy) return;
    final t = AppLocalizations.of(context);
    final messenger = ScaffoldMessenger.of(context);
    setState(() => _busy = true);
    try {
      final n = await _run(applies, op);
        ...
        child: SizedBox(
          ...
          child: Row(
            children: [
              ...
              Text(t.nSelected(widget.controller.count),
              ...
              if (_busy)
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    reverse: true,
                    child: Row(children: [
                      _action(t.markAllWatched, Icons.done_all_rounded,
                          () => _act((_) => true, (api, it) => it.kind == SelKind.series ? api.watchSeries(it.id) : api.watchMovie(it.id))),
                      if (widget.inLibrary)
                        _action(t.unfollow, Icons.remove_circle_outline_rounded,
                            () => _act(_isSeries, (api, it) => api.setFollow(it.id, false)))
                      else
                      ...
                      _action(t.stopWatching, Icons.pause_circle_rounded,
                          () => _act(_isSeries, (api, it) => api.setStatus(it.id, 'stopped'))),
